...
class Embedding():
    ...
    def __init__(self, input_dim : int, output_dim : int, data_type : type = np.float32) ->None:
    ...
    def build(self) -> None:
        ...
        self.w = np.random.normal(0, pow(self.input_dim, -0.5), (self.input_dim, self.output_dim)).astype(self.data_type) # random matrix with shape (input_dim, output_dim)

        self.v, self.m         = np.zeros_like(self.w).astype(self.data_type), np.zeros_like(self.w).astype(self.data_type) # zeros matrices with shape (input_dim, output_dim)
        self.v_hat, self.m_hat = np.zeros_like(self.w).astype(self.data_type), np.zeros_like(self.w).astype(self.data_type) # zeros matrices with shape (input_dim, output_dim)
